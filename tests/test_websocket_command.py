...
def test_pending_approval_batch_waits_until_all_decisions() -> None:
    ...
    reqs = DeferredToolRequests(
        approvals=[
            ToolCallPart(tool_name="run_minecraft_command", args={"command": "a"}, tool_call_id="tc1"),
            ToolCallPart(tool_name="run_minecraft_command", args={"command": "b"}, tool_call_id="tc2"),
        ]
    )
    now = time.time()
    batch_id = "batch-1"
    ...
    assert {item.tool_call_id: item.decision for item in done2} == {"tc1": True, "tc2": False}
    assert len(store) == 0
