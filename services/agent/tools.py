...
async def register_mcp_tools(chat_agent: Agent[AgentDependencies, str]) -> int:
    ...
    Args:
        chat_agent: PydanticAI Agent 实例
    ...
    adapters = mcp_manager.get_all_adapters()
    if not adapters:
    ...
    for server_name, adapter in adapters.items():
        ...
        for mcp_tool in mcp_tools:
            ...
            async def create_mcp_tool_wrapper(
                ...
                async def mcp_tool_wrapper(
                    ...
                    **kwargs: Any,
                    ...
                    if not client:
                        return f"MCP 客户端未连接: {server_name}"
            ...
            # 为每个工具创建一个包装器
            tool_func = await create_mcp_tool_wrapper(tool_name, server_name)
            tool_func.__name__ = f"mcp_{server_name}_{tool_name}"
            tool_func.__doc__ = tool_description

            # 使用装饰器注册工具
            chat_agent.tool(tool_func)
            registered_count += 1
            logger.debug(
                "mcp_tool_registered",
                server=server_name,
    ...
    logger.info("mcp_tools_registered", count=registered_count)
